...
const SECTION_1_PRO4X: SignalGuideItem[] = [
    ...
    },
    {
        ...
        color: '#4ADE80',
        description: 'This is the confirmed long entry signal.\n\nAll required conditions are aligned at a key level, indicating a high-probability buying opportunity.\n\nDepending on ATR volatility, the market may still push higher for a stop hunt before reversing.\n\nDuring these phases, multiple Buy alerts in the same area are normal. (most rarely)\n\nAlways read the entry price displayed in the alert carefully.\n\nWait for the market to pull back and retest this exact level for the entry to be valid.\n\nDo not enter immediately when the alert triggers.\n\nN.B. You can expect between 5 and 7 confirmed entries per US session.\n\nImportant: If no signals appear, it means market conditions did not meet the strict parameters. This is normal behavior. Do not force the market.',
    ...
    },
    {
        ...
        color: '#EF4444',
        description: 'This is the confirmed short entry signal.\n\nAll required conditions are aligned at a key level, indicating a high-probability selling opportunity.\n\nDepending on ATR volatility, the market may still push lower for a stop hunt before reversing.\n\nDuring these phases, multiple Sell alerts in the same area are normal. (most rarely)\n\nAlways read the entry price displayed in the alert carefully.\n\nWait for the market to pull back and retest this exact level for the entry to be valid.\n\nDo not enter immediately when the alert triggers.\n\nN.B. You can expect between 5 and 7 confirmed entries per US session.\n\nImportant: If no signals appear, it means market conditions did not meet the strict parameters. This is normal behavior. Do not force the market.',
...
const SECTION_2_HORUS: SignalGuideItem[] = [
    {
        ...
        icon: 'pulse',
        color: '#FFD700',
        description: 'This Get Ready signal is specific to the Horus system.\nIt indicates that the structure is forming at a key area. It\'s a different calculation of Pro4x.\nThis is a pre-signal, not a confirmed entry.\n\nA Buy or Sell Entry may follow — or not.\n\nN.B. You can expect an average of 8 signals per US session.',
        action: 'Best practice: wait for the Entry alert.',
    },
    {
        title: 'Horus — Buy Entry',
        icon: 'trending-up-outline',
        color: '#4ADE80',
        description: 'This is the confirmed long entry signal generated by Horus.\n\nAll required conditions are aligned.\n\nDue to volatility and ATR dynamics, the market may extend further for a stop hunt, triggering multiple Buy alerts in the same area.\n\nAlways read the entry price shown in the alert carefully.\n\nWait for the price to retest this exact level.\n\nDo not enter immediately when the alert triggers.\n\nN.B. You can expect an average of 8 confirmed entries per US session.',
        action: 'Works best on Key levels such as 12/23/38/64/91 with volume confirmation on your graph (Market Volume Delta).\n\nExecution discipline and patience are part of the system.\nAlerts indicate opportunity, not obligation.',
    },
    {
        title: 'Horus — Sell Entry',
        icon: 'trending-down-outline',
        color: '#EF4444',
        description: 'This is the confirmed short entry signal generated by Horus.\n\nAll required conditions are aligned.\n\nDue to volatility and ATR dynamics, the market may extend further for a stop hunt, triggering multiple Sell alerts in the same area.\n\nAlways read the entry price shown in the alert carefully.\n\nWait for the price to retest this exact level.\n\nDo not enter immediately when the alert triggers.\n\nN.B. You can expect an average of 8 confirmed entries per US session.',
        action: 'Works best on Key levels such as 12/23/38/64/91 with volume confirmation on your graph (Market Volume Delta).\n\nExecution discipline and patience are part of the system.\nAlerts indicate opportunity, not obligation.',
    },
];
...
const SECTION_4_H1: SignalGuideItem[] = [
    {
        title: 'Syncro Buy (H1) — SPX / ES / NQ',
        icon: 'caret-up-outline',
        color: '#4ADE80',
        description: 'This alert means the H1 bullish bias is synchronized across SPX, ES, and NQ.\n\nIt can be considered as a non-scalping entry for longer duration trades, PROVIDED that the market structure is clean and the next H4 Resistance is far enough to allow room.\n\nEducational: The Higher Timeframe always wins over the Lower Timeframe. A trend reversal is not confirmed until the price crosses the 200 EMA.',
        action: 'Best practice: wait for the next GR Buy and Buy Entry (or a retest of the calculated entry price) before execution.\n\nWait 15mn after market open for validity.',
    },
    {
        title: 'Syncro Sell (H1) — SPX / ES / NQ',
        icon: 'caret-down-outline',
        color: '#EF4444',
        description: 'This alert means the H1 bearish bias is synchronized across SPX, ES, and NQ.\n\nIt can be considered as a non-scalping entry for longer duration trades, PROVIDED that the market structure is clean and the next H4 Support is far enough to allow room.\n\nEducational: The Higher Timeframe always wins over the Lower Timeframe. A trend reversal is not confirmed until the price crosses the 200 EMA.',
        action: 'Best practice: wait for the next GR Sell and Sell Entry (or a retest of the calculated entry price) before execution.\n\nWait 15mn after market open for validity.',
    },
];
...
const SECTION_6_VISUALS: VisualEducationItem[] = [
    ...
    {
        title: 'PRO4X | Institutional Buy',
        subtitle: 'Validation at Support Level',
        image: require('../assets/images/education/pro4x_buy.png'),
        realCapture: require('../assets/images/wtc_2_premium.png'),
        explanation: 'LIVE EXECUTION — PRO4X INSTITUTIONAL BUY\n\nThis Buy signal is not a random alert.\nIt already displays the best statistically calculated entry price.\n\nThe PRO4X system is designed to:\n\nAnalyze liquidity\nRead market structure\nEvaluate execution probability\n\nAnd pre-compute the most likely institutional entry level.\n\nWhat you see in the alert is not a suggestion —\nit is the highest-probability number at that moment.\n\nThis Buy signal is not a fixed price entry.\nIt defines a liquidity zone, not a number.\n\nWhen a Get Ready prints — for example at 25538 —\nthe institutional entry will almost never occur exactly at that price.\n\nIt will occur on one of the execution numbers:\n\n12 / 38 / 64 / 91.\n\nYour job is to identify which one.\n\nHow institutions choose the number\n\nInstitutions do not guess.\nThey let liquidity decide.\n\nAfter a GR at 25538, price may rotate to:\n25564\n25591\nor sweep to 25512\n\nEach number corresponds to a different market intention.\n\nScenario 1 — Strong and persistent H1 trend\n\nIf the H1 trend is strong and has been active for a while:\nInstitutions do not need a deep stop-hunt\nLiquidity is already present\nPrice often reacts early\n\nIn this case, prioritize the 64\nExecution happens higher, without going to 91 or 12.\n\nThis is continuation-style accumulation.\n\nScenario 2 — Late trend or exhaustion on H1\n\nIf the H1 trend is overextended or losing momentum:\nInstitutions need more liquidity\nA stop-hunt is likely\n\nPrice may push to 91\nor even sweep down to 12\nbefore the real reversal starts.\n\nThis is where weak hands are forced out.\n\nScenario 3 — Curve-based execution\n\nIf price is already well positioned on the curve:\nNo imbalance\nNo emotional pressure\nVolume is balanced (VRVP)\n\nExecution can occur directly on the curve,\nmost often around 38 or 64,\nwithout any extreme move.\n\nClean. Controlled. Institutional.\n\nWhat to check before execution\n\nBefore choosing the number, you must read:\nH1 trend strength\nCurve position\nVRVP structure (where liquidity is thick)\nPrice behavior near each level\n\nThe number is not random.\nIt is selected by context.',
        proTip: '**KEY RULE**\n\nDo not buy because a signal appears.\nBuy where institutions are forced to act.\n\nIf you don’t understand the numbers,\nyou are guessing.\n\nIf you do,\nyou are trading with the execution model.',
        videoModules: {
            buy: require('../assets/videos/pro4x_buy.mp4'),
            sell: require('../assets/videos/pro4x_ex2.mp4')
        },
    ...
    {
        title: 'PRO4X | Institutional Sell',
        subtitle: 'Precision at Resistance',
        image: require('../assets/images/education/pro4x_sell.png'),
        realCapture: require('../assets/images/wtc_3_premium.png'),
        explanation: 'LIVE EXECUTION — PRO4X INSTITUTIONAL SELL\n\nThis Sell signal is not a random alert.\nIt already displays the best statistically calculated sell entry price.\n\nThe PRO4X system is designed to:\n\nAnalyze buy-side liquidity\nRead market structure\nEvaluate execution probability\nPre-compute the most likely institutional sell level\n\nWhat you see in the alert is not a suggestion —\nit is the highest-probability sell number at that moment.\n\nThis Sell signal is not a fixed price\n\nIt defines a liquidity zone, not a single number.\n\nWhen a Get Ready prints — for example at 25538 —\nthe institutional sell will almost never occur exactly at that price.\n\nExecution will occur on one of the institutional numbers:\n\n12 / 38 / 64 / 91\n\nYour job is to identify which one will be respected.\n\nHow institutions choose the sell number (NQ logic)\n\nInstitutions do not guess.\nThey let buy-side liquidity build, then sell into it.\n\nAfter a GR at 25538, price may rotate to:\n25564\n25591\nor spike to 25512 before rejection\n\nEach number reflects a different distribution intent.\n\nScenario 1 — Strong and persistent H1 uptrend (Nasdaq typical)\n\nOn the NQ, strong H1 trends are often violent and extended.\n\nIf the H1 uptrend is clean and still strong:\nInstitutions do not need a deep breakout\nLate buyers are already aggressive\nDistribution starts early\n\nIn this case, prioritize the 64\nSell execution happens below the extreme,\noften without reaching 91.\n\nThis is early distribution into strength.\n\nScenario 2 — Overextended H1 trend / euphoric phase\n\nIf the H1 trend is:\nOverextended\nVertical\nFueled by FOMO (very common on NQ)\n\nInstitutions need maximum buy-side liquidity\n\nPrice often pushes to 91\nor briefly spikes before rejection.\n\nThis is the classic Nasdaq stop-hunt above highs\nbefore a sharp sell-off.\n\nScenario 3 — Curve-based sell execution\n\nIf price is already:\nHigh on the curve\nMomentum is slowing\nVRVP shows heavy volume above\n\nExecution can occur directly on the curve,\nmost often around 38 or 64,\nwithout any final spike.\n\nClean. Silent. Institutional.\n\nWhat to validate before selling\n\nBefore executing the Sell, confirm:\nH1 trend strength or exhaustion\nCurve position\nVRVP liquidity above price\nPrice behavior at each number\n\nThe number is never random.\nIt is selected by context and liquidity.',
        proTip: '**KEY RULE**\n\nDo not sell because a signal appears.\nSell where institutions distribute risk.\n\nIf you don’t understand the numbers,\nyou are reacting.\n\nIf you do,\nyou are executing the PRO4X institutional model.',
        videoUrl: require('../assets/videos/pro4x_sell.mov'),
        videoAspectRatio: 1.6,
        videoCaption: 'PRO4X EXECUTION'
    },
                                    ...
                                    <View style={{ flexDirection: 'row', justifyContent: 'center', marginTop: 10, marginBottom: 10, gap: 10 }}>
                                        ...
                                        >
                                            ...
                                            <Text style={[styles.masterclassBadgeText, { color: videoTab === 'buy' ? '#000' : '#888', marginLeft: 5 }]}>
                                                {videoTabLabels ? videoTabLabels.buy : 'BUY SCENARIO'}
                                            </Text>
                                        </TouchableOpacity>
                                        <TouchableOpacity
                                        ...
                                        >
                                            ...
                                            <Text style={[styles.masterclassBadgeText, { color: videoTab === 'sell' ? '#FFF' : '#888', marginLeft: 5 }]}>
                                                {videoTabLabels ? videoTabLabels.sell : 'SELL SCENARIO'}
                                            </Text>
                                        </TouchableOpacity>
                                    </View>
                                )}
